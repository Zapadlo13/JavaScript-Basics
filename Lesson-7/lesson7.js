...
function showScore() {
    ...
    h.innerText = 'Ваш результат:' + score;
    document.getElementById('snake-score').innerHTML = ""; // Очищаем
    document.getElementById('snake-score').appendChild(h); // Добавление счет
}
...
function move() {
    ...
    if (direction == 'x-') {
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (--coord_x < 0 ? FIELD_SIZE_X - 1 : coord_x))[0];
    }
    else if (direction == 'x+') {
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (++coord_x > FIELD_SIZE_X - 1 ? 0 : coord_x))[0];
    }
    else if (direction == 'y+') {
        new_unit = document.getElementsByClassName('cell-' + (--coord_y < 0 ? FIELD_SIZE_Y - 1 : coord_y) + '-' + (coord_x))[0];
    }
    else if (direction == 'y-') {
        new_unit = document.getElementsByClassName('cell-' + (++coord_y > FIELD_SIZE_Y - 1 ? 0 : coord_y) + '-' + (coord_x))[0];
    }

    // Проверки
    // 1) new_unit не часть змейки
    // 2) Змейка не ушла за границу поля
    //console.log(coord_y, coord_x, new_unit);
...
function createWall() {
    ...
    while ((!wallCreated) && (gameIsRunning)) { //пока еду не стену
        // рандом
